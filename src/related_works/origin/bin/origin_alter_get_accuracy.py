...
def getParameters():
    ...
    parser.add_argument("--unseenbinlist", help="Input of text file each line one binary to predict")
...
def PrintPrediction(labelString, predict, addrList):
    ...
    for label in range(1, len(labelString) + 1):
	    ...
	    column[label] = col
	    col += 1
	    predicted_provenance.append(labelString[label])
...
unseen_binaryfiles = args.unseenbinlist
#files = open(file
...
predicted = 0
w_out_openssl = 0
for binpath in unseen_binaryfiles:
	#print binpath
	count = count + 1
	binpath = binpath.strip()
	filename = binpath.split("/")[-1]
	
	#segmentation error generating 
	if "openssl" in binpath or "bash-4.3.mksyntax.gcc-4.8.4.O0" in binpath:
		w_out_openssl = w_out_openssl + 1
		continue
	
		
	#print filename
	gt_bin_comp, gt_bin_version,gt_bin_olvl = getGroundTruth(filename)
	gt_provenance = gt_bin_comp +"-"+ gt_bin_version +"-"+ gt_bin_olvl 
	labelString = LoadLabelString()
	 
	if gt_bin_comp!="" and gt_bin_version != "" or gt_bin_olvl !="":
		print "----------------------"
		#print filename
		#print binpath
		#print "GT: "+ gt_bin_comp +"-"+ gt_bin_version +"-"+ gt_bin_olvl 
		
		GenerateFeatures("idiom", idiomSizes, binpath, filename)
		GenerateFeatures("graphlet", graphletSizes, binpath, filename)
		endAddr = GetEndAddress(binpath, filename)
		#addrList = []
		featureIndex, featureScale = LoadFeatureData()
		testDataFileName, addrList = GenerateCRFData(binpath)
		...
		#print "addrList size: "+ str(len(addrList))

		predicted_provenance = PrintPrediction(labelString, predict, addrList)
		
		print "Ground Truth: "+gt_provenance +"\nPredicted Labels: "
		for prov in predicted_provenance:
			print prov
			if gt_provenance.strip() == prov.strip():
				#print "Truely Predicted"
				predicted = predicted + 1 
		
		
		print "Binary Processed: "+str(count)
		print "Predicted: "+str(predicted) 
		
		#if count >=400:
		#	break
print "Total Predicted: "+str(predicted)
print "OPEN_SSL Binaries: "+str(w_out_openssl)
print "Total #of Predication: "+str(count)
accuracy = float(predicted*100.0/(count - w_out_openssl))
