...
def main():
        ...
        if not args.input_model:
                ...
                for f in files_file:
                        ...
                        if file_type in file_types:
                                ...
                                num_of_file_types[file_type] = 1
                                #print( "Else: ",num_of_types,file_type)
                                num_of_types+=1
                #print(f.replace(path,"").replace(os.path.basename(f),""))#.split("/",1)[0])
                #print("num_of_types: ",num_of_types)
                #print("num_of_types: ",num_of_types)
                ...
                #print("order_l",len(order_l),len(order_l[0]))
                #assert(ass_ert_debug!=0)
                #print("files_file: ",files_file[1:5])
                random.shuffle(files_file)
                #print("files_file: ",files_file[1:5])

                no_of_files_processed = 0
                for f in files_file:
                        print("LINE 323: File in Processing ",f)
                        ...
                        fsize = len(bdata)
                        if fsize < block_size:
                                print("Line 363 File Not Processed: ",f)
                                continue

                        no_of_files_processed +=1
                        print("Line 367 File Processed: ", no_of_files_processed)
                        ...
                        for c in range(0,fsize-block_size,block_size):
                                ...
                                for x in block:
                                        #print(chr(x))
                                        train.extend(BitArray[ord(chr(x))])
                ...
                for t in file_types_:
                        ...
                        if args.disasm_x86:
                                for j in range(1,16+1):
                                        ord_lstr += str(order_l[file_types[t]][j]) + " "
                        print( t, num_of_file_types[t], num_of_dataset[t], ord_lstr)
                        #print( "")
        ...
        if args.output_model and master_dataset:
                ...
                # iteration, which will be used by the print(Report extension below.
                ...
                for i in range(0,fsize):
                        ...
                        if b == 0x00:
                                ...
                                c=(0,0,0)
                        img.putpixel((i%128,int(i/128)),c)
                if output_image:
                ...
                img_ = Image.new('RGB', (128, int(h)))
                #解析対象のファイルの分類結果を表示
                img = [Image.new('RGB', (128, int(h))) for i in range(len(col))]
                #sliding window is changed to argument length
                l=args.length
                ...
                for c in range(0,fsize-block_size+1,l):
                        offset = c*1.0/fsize
                        block = bdata[c:c+block_size]
                        block_ = [ord(chr(x)) for x in block]
                        e = entropy(block_)
                        for j in range(0,l):
                                img_.putpixel(((c+j)%128,int((c+j)/128)),(e,e,e))
                        if args.disasm_x86:
                                ...
                                for i in m:
                                        ...
                                        for c_ in range(16):#16バイトで命令を切る
                                                if c_ < len(i[3])/2:
                                                        b += (str((i[3][c_*2:c_*2+2],16)).encode('ascii'))
                                                else:
                                                        b += b'\0'
                                        block += b
                        ...
                        for j in range(0,l):
                                for i in range(len(col)):
                                        img[i].putpixel(((i_*l+j)%128,int((i_*l+j)/128)),col[i][result2])
                        i_+=1
                        if output_image:
                            if (i_%128) == 0:
                                    box = (0,num*max_h,128,num*max_h+max_h)
                ...
                #k-分割交差検証
                master_dataset,num_of_dataset,file_types_, file_types= make_dataset(args,block_size)
                print("Dataset Length: ",len(master_dataset[1]))
                ...
                for i in range(k):
                        ...
                        for train in master_dataset:
                                ...
                                #print("Samples: ",file_types_[ft]," ",totalsamples)
                        ...
                        for t in file_types_:
                                ...
                                print( ft,t,ftn[ft],tp[ft],fp[ft],fn[ft],tn[ft],r,p,f1,acc)
                ...
                for t in file_types_:
                        ...
                        print(ft,t,mftn[ft],mtp[ft],mfp[ft],mfn[ft],mtn[ft],p,r,f1,acc)
...
def make_dataset(args,block_size):
        if args.input_model:
                #datasetディレクトリから学習モデルを作成
                ...
                for f in files_file:
                        ...
                        fsize = len(bdata)
                        if fsize < block_size:
                                print("Line 1019: File Not Processed: ",f)
                                continue

                        no_of_files_processed +=1
                        print("Line 1023 File Processed: ", no_of_files_processed)
                        #block_size(256バイト)区切りでデータセット作成
                        for c in range(0,fsize-block_size,block_size):
                                if args.s_limit > 0 and num_of_dataset[ft] >= args.s_limit:
                                        break
                ...
                total_files = 0
                total_types = 0
                print("LINE 1046 FILE TYPES: ",file_types_)
                ...
                print(('# epoch: {}'.format(args.epoch)))
                print((''))
        return master_dataset,num_of_dataset,file_types_,file_types
